...
function create_banlist()
{
    ...
    $result = db_query ("SELECT ban_id,user_id,ip_addr,unix_timestamp(expiry) as expiry FROM {$CONFIG['TABLE_BANNED']}");
...
if (count($HTTP_POST_VARS) > 0) {
    if (isset($HTTP_POST_VARS['add_ban'])) {
        ...
        if ($ban_uid || $ban_ip_addr) {
            db_query("INSERT INTO {$CONFIG['TABLE_BANNED']} (user_id, ip_addr, expiry) VALUES ($ban_uid, $ban_ip_addr, from_unixtime($ban_expires))");
        } else {
        ...
        if (isset($HTTP_POST_VARS['ban_id'])) {
            ...
            if ($ban_id) {
                ...
                if ($ban_uid || $ban_ip_addr) {
                    db_query("UPDATE {$CONFIG['TABLE_BANNED']} SET user_id=$ban_uid, ip_addr=$ban_ip_addr, expiry=from_unixtime($ban_expires) where ban_id=$ban_id");
                } else {
                    cpg_die(CRITICAL_ERROR, "You need to specifiy either a user name or an IP address.", __FILE__, __LINE__);
                }
            } else {
...
$signature = 'Netfirms Picture Pro Photo Gallery ' . COPPERMINE_VERSION;
